...
public class ReactionServer {
    ...
    public Map<String, Object> like(Long post_id, User user) throws UserExeption {
        if (post_id == 0) {
            throw new UserExeption(400, "Post not found");
        }
        Post post = postRepositery.findById(post_id).orElseThrow(() -> new UserExeption(400, "Post not found"));
        if (post.isHide()) {
            throw new UserExeption(400, "You can't react in this post");
        }
